...
```console
$ docker run --name some-mongo -d rodinvr/mongo
```
...
```console
$ docker run -it --link some-mongo:mongo --rm rodinvr/mongo sh -c 'exec mongo "$MONGO_PORT_27017_TCP_ADDR:$MONGO_PORT_27017_TCP_PORT/test"'
```
...
```console
$ docker run --name some-mongo -d rodinvr/mongo --storageEngine wiredTiger
```
...
```console
$ docker run --name some-mongo -d rodinvr/mongo --auth
```

#### Add the Initial Admin User

```console
$ docker exec -it some-mongo rodinvr/mongo admin
connecting to: admin
...
```console
$ docker run -it --rm --link some-mongo:mongo rodinvr/mongo mongo -u jsmith -p some-initial-password --authenticationDatabase admin some-mongo/some-db
> db.getName();
some-db
```
...
2.	Start your `rodinvr/mongo` container like this:

	```console
	$ docker run --name some-mongo -v /my/own/datadir:/data/db -d rodinvr/mongo:tag
	```
